			...
			<div class="body">
				<div class="inner-body">
							...
							<div class="panel-data">
								<ul>
									{foreach from=$default_pages item=default_page}
										<li class="category">
											<div class="services category" id="d_{$default_page.id}" onclick="window.location='{$base_url}Auth/Pages/viewPage?type={$default_page.type}&id={$default_page.id}&name={$default_page.name}'">{$default_page.text}</div>																					
										</li>
									{/foreach}
									{foreach from=$generated_pages item=generated_page}
										<li class="category">
											<div class="services category {if $id == $generated_page.id}category_selected{/if}" id="g_{$generated_page.id}" onclick="window.location='{$base_url}Auth/Pages/viewPage?action=edit&type={$generated_page.type}&id={$generated_page.id}&name={$generated_page.name}'">{$generated_page.text}</div>																					
										</li>
									{/foreach}
								</ul>
							</div>
					...
					<div class="users-panel general-data">
						<form method="POST" action='{$base_url}Auth/Pages/{if $action == "edit"}editPage{else}addPage{/if}?action={$action}{if isset($id) && $id != ""}&id={$id}{/if}{if isset($type) && $type != ""}&type={$type}{/if}' enctype='multipart/form-data'>
							<div class="inner-panel">
								<div class="add buttons">
									<input type="submit" value="Save" name="action"> | 
									{if $action == 'edit'}
										<input type="button" value="Remove" name="remove" onclick=" removePage('{$base_url}',{$id})"> |
									{/if}
