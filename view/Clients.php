...
<link href="{$css}/clients.css" type="text/css" rel="stylesheet" />
        ...
        <ul class="verticalMenu">
          {foreach from=$services item=categories}
                ...
                {foreach from=$categories.subcat item=subcategories}
                    <li><a href="{$base_url}Home/service?cat_id={$categories.cat_id}&subcat_id={$subcategories.subcat_id}" id="subcat_{$subcategories.subcat_id}_{$categories.cat_id}">{$subcategories.subcat_name}</a></li>
                {/foreach}
            {/foreach}                      
        </ul>
        <!--contact widget-->
                    ...
                    <li>
                        ...
                        <ul>
                            {foreach from=$implementations item=implementation}
                                <li>
                                    <div class="company_name">{$implementation.type_name}</div>
                                    {foreach from=$implementation.clients item=client}
                                        <div class="company_name" style="text-indent:20px;">{$client.name}</div>
                                        {if $client.location != ''}
                                            <div class="location" style="text-indent:20px;">{$client.location}</div>
                                        {/if}
                                    {/foreach}                               
                                </li>
                            {/foreach} 
                    ...
                    <li>
                        ...
                        <ul>
                            {foreach from=$clients item=client}
                                <li>
                                    <div class="company_name">{$client.company_name}</div>
                                    <div class="location">{$client.address}</div>

                                    {if isset($client.contact_person)}
                                        <div class="contact">
                                            <u><strong>{$client.contact_person} {if isset($client.position)}- {$client.position}{/if}</strong></u>
                                        </div>
                                    {/if}
                                    
                                    {if isset($client.contact_number)}
                                        <div class="contact_num">{$client.contact_number}</div>                                
                                    {/if}
                                </li>
                            {/foreach}
                        </ul>
